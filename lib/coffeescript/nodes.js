  ...
  exports.Block = Block = (function() {
    class Block extends Base {
      ...
      extractTypeSignatures() {
        var declaredIdentifier, declaredName, expression, expressions, index, j, len1, nextExpression, results1, typeSignatureName;
        expressions = this.expressions.slice(0);
        results1 = [];
        for (index = j = 0, len1 = expressions.length; j < len1; index = ++j) {
          expression = expressions[index];
          if (!(expression instanceof TypeSignature)) {
            continue;
          }
          nextExpression = expressions[index + 1];
          if (!(nextExpression instanceof VariableDeclaration)) {
            expression.error('Type signature must be followed by corresponding declaration');
          }
          ...
          if (typeSignatureName !== declaredName) {
            expression.error(`Type signature for ${typeSignatureName} doesn't match following declaration for ${declaredName}`);
          }
          if (declaredIdentifier.typeAnnotation != null) {
            expression.error("Type signature can't be applied to declaration with existing type annotation");
          }
          declaredIdentifier.typeAnnotation = expression.typeAnnotation;
          results1.push(this.expressions.splice(index, 1));
        }
        return results1;
      }
  ...
  exports.IdentifierLiteral = IdentifierLiteral = (function() {
    class IdentifierLiteral extends Literal {
      ...
      compileNode(o) {
        var code;
        code = [];
        code.push(this.makeCode(this.value));
        if (this.typeAnnotation != null) {
          code.push(this.makeCode(': '));
          code.push(...this.typeAnnotation.compileNode(o));
        }
        return code;
      }
...
// multiline need to be indented.
  ...
  exports.VariableDeclaration = VariableDeclaration = (function() {
    class VariableDeclaration extends Base {
      ...
      compileNode(o) {
        var code, declaration, index, j, len1, ref1;
        code = [];
        code.push(this.makeCode(`${this.tab}${this.kind} `));
        ref1 = this.declarations;
        for (index = j = 0, len1 = ref1.length; j < len1; index = ++j) {
          declaration = ref1[index];
          code.push(...declaration.compileToFragments(o));
          if (index !== this.declarations.length - 1) {
            code.push(this.makeCode(", "));
          }
        }
        return code;
    ...
    VariableDeclaration.prototype.children = ['declarations'];

    return VariableDeclaration;

  }).call(this);

  exports.VariableDeclarator = VariableDeclarator = (function() {
    class VariableDeclarator extends Base {
      ...
      compileNode(o) {
        var code;
        code = [];
        code.push(...this.id.compileNode(o));
        if (this.init != null) {
          code.push(this.makeCode(' = '));
          code.push(...this.init.compileToFragments(o));
        }
        return code;
      }
    ...
    VariableDeclarator.prototype.children = ['id', 'init'];

    return VariableDeclarator;

  }).call(this);

  exports.TSTypeAnnotation = TSTypeAnnotation = (function() {
    class TSTypeAnnotation extends Base {
      constructor(typeAnnotation) {
        super();
        this.typeAnnotation = typeAnnotation;
        if (this.typeAnnotation instanceof IdentifierLiteral && this.typeAnnotation.value === 'number') {
          this.typeAnnotation = new TSNumberKeyword();
        }
      }

      compileNode(o) {
        return this.typeAnnotation.compileToFragments(o);
      }

    };

    TSTypeAnnotation.prototype.children = ['typeAnnotation'];

    return TSTypeAnnotation;

  }).call(this);

  exports.TSNumberKeyword = TSNumberKeyword = class TSNumberKeyword extends Base {
    compileNode() {
      return [this.makeCode('number')];
    }

  };

  exports.TypeSignature = TypeSignature = (function() {
    class TypeSignature extends Base {
      constructor({
          name: name1,
          typeAnnotation
        }) {
    ...
    TypeSignature.prototype.children = ['name', 'typeAnnotation'];
